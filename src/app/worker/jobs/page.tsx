...
const TABS: Array<{ key: JobTab; label: string; icon: LucideIcon }> = [
...
function getJobGroup(status: JobStatus): Exclude<JobTab, "all"> | null {
  if (status === JobStatus.PENDING_ACCEPTANCE) return "pending";
  if ([JobStatus.ACCEPTED, JobStatus.ON_THE_WAY, JobStatus.ARRIVED, JobStatus.IN_PROGRESS].includes(status)) return "active";
  if ([JobStatus.COMPLETED, JobStatus.CANCELLED, JobStatus.REJECTED].includes(status)) return "history";
  return null;
}
...
function getStatusMeta(status: JobStatus) {
  switch (status) {
    case JobStatus.PENDING_ACCEPTANCE:
      return { label: "Pending", bg: "#E85D4C1A", fg: "#E85D4C", icon: Clock };
    case JobStatus.ACCEPTED:
      return { label: "Accepted", bg: "#2231591A", fg: "#223159", icon: Briefcase };
    case JobStatus.ON_THE_WAY:
      return { label: "On the way", bg: "#D9A4411A", fg: "#D9A441", icon: RefreshCw };
    case JobStatus.ARRIVED:
      return { label: "Arrived", bg: "#8B5CF61A", fg: "#8B5CF6", icon: MapPin };
    case JobStatus.IN_PROGRESS:
      return { label: "In progress", bg: "#D9A4411A", fg: "#D9A441", icon: Loader2 };
    case JobStatus.COMPLETED:
      return { label: "Completed", bg: "#1FA97E1A", fg: "#1FA97E", icon: CheckCircle2 };
    case JobStatus.CANCELLED:
      return { label: "Cancelled", bg: "#E85D4C1A", fg: "#E85D4C", icon: Clock };
    case JobStatus.REJECTED:
      return { label: "Rejected", bg: "#E85D4C1A", fg: "#E85D4C", icon: Clock };
    default:
      return { label: status.replaceAll("_", " "), bg: "#EEECE4", fg: "#5B5F6B", icon: Briefcase };
  }
}
...
function StatusStamp({ status }: { status: JobStatus }) {
  ...
  return (
    ...
    >
      <Icon size={12} className={status === JobStatus.IN_PROGRESS ? "animate-spin" : ""} strokeWidth={2.6} />
      {meta.label}
    </span>
  );
}

function RingProgress({ pct, size = 132 }: { pct: number; size?: number }) {
  const radius = 52;
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => setDashOffset(circumference - (pct / 100) * circumference), 150);
  ...
  return (
    <div className="relative shrink-0" style={{ width: size, height: size }}>
      <svg viewBox="0 0 120 120" className="h-full w-full -rotate-90">
        <circle cx="60" cy="60" r={radius} fill="none" stroke="#EEECE4" strokeWidth="10" />
        <circle
          cx="60"
          cy="60"
          r={radius}
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <span className="font-mono text-2xl font-semibold tabular-nums text-[#171A21]">{count}%</span>
        <span className="text-[10px] font-semibold uppercase tracking-[0.1em] text-[#5B5F6B]">Completed</span>
      </div>
    </div>
  );
}
    ...
    <div className="flex h-24 items-end justify-between gap-2">
      {data.map((value, index) => (
        <div key={index} className="flex flex-1 flex-col items-center gap-2">
          ...
          <span className="text-[10px] font-semibold text-[#5B5F6B]">{labels[index]}</span>
        </div>
      ))}
    </div>
  );
...
function MetricItem({ label, value, color, delay }: { label: string; value: number; color: string; delay: number }) {
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div>
        <p className="font-mono text-xl font-semibold leading-none tabular-nums">{count}</p>
        <p className="mt-1 text-xs text-[#5B5F6B]">{label}</p>
      </div>
    </div>
  );
}
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between">
        <span className="text-[11px] font-semibold uppercase tracking-[0.12em] text-[#5B5F6B]">{label}</span>
      ...
      <div className="mt-6 font-mono text-3xl font-semibold tabular-nums text-[#171A21]">
        {prefix && <span className="mr-0.5 align-top text-lg text-[#5B5F6B]">{prefix}</span>}
...
function TabBar({ active, onChange, counts }: { active: JobTab; onChange: (tab: JobTab) => void; counts: Record<JobTab, number> }) {
  ...
  return (
    <div ref={containerRef} className="relative flex items-center gap-1 overflow-x-auto border-b border-[#E4E1D8] no-scrollbar">
      {TABS.map((tab, index) => {
        const Icon = tab.icon;
        ...
        return (
          <button
            ...
            className={`flex items-center gap-2 whitespace-nowrap px-4 py-3 text-sm font-medium transition-colors ${
              isActive ? "text-[#171A21]" : "text-[#5B5F6B] hover:text-[#171A21]"
            }`}
          >
            ...
            <span
              className={`min-w-[20px] rounded-full px-1.5 py-0.5 text-center text-[11px] font-semibold transition-colors ${
                isActive ? "bg-[#223159] text-white" : "bg-[#EEECE4] text-[#5B5F6B]"
              }`}
            >
              {counts[tab.key]}
            </span>
      ...
      <span className="absolute bottom-0 h-[2px] rounded-full bg-[#D9A441] transition-all duration-300 ease-out" style={{ left: indicator.left, width: indicator.width }} />
    </div>
  );
}

function JobCard({ job, index, onAction }: { job: WorkerJob; index: number; onAction?: (jobId: string, action: JobAction["key"]) => void }) {
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl" style={{ backgroundColor: `${meta.fg}1A` }}>
            <Briefcase size={18} color={meta.fg} strokeWidth={2.2} />
          </span>
          <div>
            <h3 className="text-lg font-semibold text-[#171A21]">{job.service.name}</h3>
            <p className="text-sm text-[#5B5F6B]">{job.service.category || "General Service"}</p>
          </div>
        </div>
      ...
      <div className="mt-5 grid grid-cols-1 gap-5 border-t border-dashed border-[#E4E1D8] pt-5 sm:grid-cols-3">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <p className="text-[11px] font-semibold uppercase tracking-[0.1em] text-[#5B5F6B]">Customer</p>
            <p className="truncate text-sm font-medium text-[#171A21]">{job.customer.name}</p>
          </div>
        </div>

        <div className="flex items-center gap-3">
          <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#E85D4C]/10">
            <MapPin size={15} className="text-[#E85D4C]" />
          </span>
          <div className="min-w-0">
            <p className="text-[11px] font-semibold uppercase tracking-[0.1em] text-[#5B5F6B]">Location</p>
            <p className="truncate text-sm font-medium text-[#171A21]">{formatLocation(job)}</p>
          </div>
        </div>

        <div className="flex items-center gap-3">
          <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#1FA97E]/10">
            <IndianRupee size={15} className="text-[#1FA97E]" />
          </span>
          <div className="min-w-0">
            <p className="text-[11px] font-semibold uppercase tracking-[0.1em] text-[#5B5F6B]">Budget payout</p>
            <p className="font-mono text-sm font-semibold text-[#171A21]">₹{formatCurrency(job.budget)}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid transition-[grid-template-rows] duration-300 ease-out" style={{ gridTemplateRows: open ? "1fr" : "0fr" }}>
        <div className="overflow-hidden">
          <p className="pt-4 pr-4 text-sm leading-relaxed text-[#5B5F6B]">{job.description || "No job description was provided for this task."}</p>
        </div>
      </div>
      ...
      <div className="mt-6 flex flex-col gap-4 border-t border-dashed border-[#E4E1D8] pt-5 sm:flex-row sm:items-center sm:justify-between">
        <JobActions status={job.status} onAction={(action) => onAction?.(job.id, action)} />
...
function EmptyState({ tab }: { tab: Exclude<JobTab, "all"> }) {
  const copy = {
    pending: {
  ...
  return (
    <div className="reveal flex flex-col items-center justify-center rounded-2xl border border-dashed border-[#E4E1D8] bg-white px-6 py-16 text-center">
      <span className="mb-4 flex h-12 w-12 animate-bounce-slow items-center justify-center rounded-full bg-[#F7F6F2]">
        <Inbox size={20} className="text-[#5B5F6B]" />
      </span>
      <p className="font-semibold text-[#171A21]">{copy.title}</p>
      <p className="mt-1 max-w-xs text-sm text-[#5B5F6B]">{copy.body}</p>
    </div>
  );
}
...
function ErrorState({ message, onRetry }: { message: string; onRetry: () => void }) {
  return (
    <div className="flex min-h-[50vh] flex-col items-center justify-center p-6 text-center">
      ...
      <h2 className="text-xl font-bold text-[#171A21]">Failed to load jobs</h2>
...
export default function WorkerJobsPage() {
  ...
  async function handleAction(jobId: string, action: JobAction["key"]) {
    if (action === "view") {
      return;
    }
  ...
  const groupedJobs = useMemo(() => {
    const pending = jobs.filter((job) => job.status === JobStatus.PENDING_ACCEPTANCE);
    const active = jobs.filter((job) => [JobStatus.ACCEPTED, JobStatus.ON_THE_WAY, JobStatus.ARRIVED, JobStatus.IN_PROGRESS].includes(job.status));
    const history = jobs.filter((job) => [JobStatus.COMPLETED, JobStatus.CANCELLED, JobStatus.REJECTED].includes(job.status));

    return { all: jobs, pending, active, history };
  }, [jobs]);

  const filteredJobs = groupedJobs[activeTab];
  ...
  const totalPayout = jobs.reduce((sum, job) => sum + job.budget, 0);
  const averageJobValue = jobs.length > 0 ? Math.round(totalPayout / jobs.length) : 0;
  const completionCount = useCountUp(completionRate, 900);

  const weekData = useMemo(() => {
    return WEEK_LABELS.map((_, index) => jobs.filter((job) => isRecentWeek(job.createdAt, index)).length);
  }, [jobs]);
  ...
  const heroValue = useCountUp(
    activeTab === "pending"
      ? groupedJobs.pending.reduce((sum, job) => sum + job.budget, 0)
      : activeTab === "active"
        ? groupedJobs.active.reduce((sum, job) => sum + job.budget, 0)
        : activeTab === "history"
          ? groupedJobs.history.reduce((sum, job) => sum + job.budget, 0)
          : totalPayout,
    900,
  );
  ...
  if (error) {
    return <ErrorState message={error} onRetry={refreshWithSpin} />;
  }
    ...
    <div className="relative space-y-6 overflow-hidden pb-12">
      <style>{`
        ...
        @media (prefers-reduced-motion: reduce) {
          .reveal, .spark-draw, .animate-bounce-slow, .drift {
      ...
      <header className="reveal relative flex flex-col gap-4 rounded-3xl border border-[#E4E1D8] bg-white/90 p-5 shadow-[0_14px_40px_-26px_rgba(23,26,33,0.25)] backdrop-blur" style={{ animationDelay: "0ms" }}>
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <div className="flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.12em] text-[#5B5F6B]">
              <Search size={13} /> Worker jobs
            </div>
            <h1 className="mt-2 text-2xl font-semibold tracking-tight text-[#171A21] md:text-[28px]">My assigned jobs</h1>
            <p className="mt-1 max-w-2xl text-sm text-[#5B5F6B]">Accept pending requests, coordinate tasks, and track job history.</p>
          </div>
      ...
      <section className="grid gap-4 lg:grid-cols-3">
        <div className="reveal relative overflow-hidden rounded-3xl border border-[#E4E1D8] bg-white p-6 shadow-[0_14px_40px_-26px_rgba(23,26,33,0.25)] lg:col-span-2" style={{ animationDelay: "40ms" }}>
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <div className="flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.12em] text-[#5B5F6B]">
                <Briefcase size={13} /> Job completion
              </div>
              ...
              {TABS.map((tab) => (
                <button
                  ...
                  className={`rounded-xl px-3.5 py-1.5 text-xs font-semibold capitalize transition-all ${
                    activeTab === tab.key ? "bg-[#223159] text-white shadow-sm" : "text-[#5B5F6B] hover:text-[#171A21]"
                  }`}
                >
                  {tab.key}
                </button>
          ...
          <div className="mt-6">
            <RingProgress pct={completionRate} />
          </div>
        </div>

        <div className="reveal flex flex-col items-center justify-center gap-3 rounded-3xl bg-[#223159] p-6 text-white" style={{ animationDelay: "90ms" }}>
          <div className="flex items-center gap-2 self-start text-[11px] font-semibold uppercase tracking-[0.12em] text-white/60">
            <Sparkles size={14} /> Jobs this week
          </div>
          <WeekChart data={weekData} labels={WEEK_LABELS} />
        </div>
      </section>

      <section className="grid gap-4 sm:grid-cols-3">
        <StatTicket label="Total payout potential" value={totalPayout} prefix="₹" icon={IndianRupee} accent="#D9A441" index={0} />
        <StatTicket label="Avg. job value" value={averageJobValue} prefix="₹" icon={Wallet} accent="#223159" index={1} />
        <StatTicket label="Completion rate" value={completionRate} icon={CheckCircle2} accent="#1FA97E" index={2} />
      </section>
      ...
      <section className="space-y-4">
        {filteredJobs.length === 0 ? (
          ...
          filteredJobs.map((job, index) => <JobCard key={`${activeTab}-${job.id}`} job={job} index={index} onAction={handleAction} />)
        )}
      </section>
    </div>
  );
